...
class Explorer:
    def __init__(self):
        ...
        self.angular_speed_HalfCircle=0.5
        self.threshold= 1
    ...
    def PID_WallFollower(self,SideDist):
        ...
        output_SideWall = proportional + self.constrain(integral,-0.1,0.1) + differential  # Calculate the result
    ...
    def obstacle_avoidance(self):
        ...
        if self.distance[0]>self.thres_obstacle:
            print("right_upper",self.LaserData.right_upper)
            print("right_upper_thres",self.SideWallDistance/cos_degrees+1.4)
            if self.isobj==1:
    ...
    def HalfCircle(self):
        ...
        while rospy.Time.now().to_sec() - t0 < turn_duration:
            ...
            if (self.LaserData.front_obs>self.thres_obstacle)&(self.LaserData.right_upper>self.SideWallDistance):
                self.cmd_vel_pub.publish(self.move_cmd)
                rate.sleep()
                print("1800")
            else:
